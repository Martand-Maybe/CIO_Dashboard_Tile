<template>
  <div class="cloud-infra-ops-page">
    <h1 class="page-title" style="text-align: left; margin-bottom: 2rem;">Cloud Infrastructure Operations</h1>
    <div class="tiles-row">
      <div
...
export default {
  ...
  data() {
    return {
      ...
      tiles: [
        {
          title: 'Critical Cloud Service Health',
          value: 5,
          prompts: [
            'What is the current health status of our critical cloud services (e.g., specific VMs, databases, Kubernetes clusters)?',
            'Were there any health degradations or outages for critical cloud services in the last 24 hours?',
            'If any critical cloud service is currently impacted, what is the customer/business impact and ETA for full service restoration?',
            'Show the health check results for [Specific Critical Cloud Service] from the last hour.',
            'What was the availability percentage for our primary cloud database service yesterday?'
          ]
        },
        {
          title: 'Monitoring beyond thresholds',
          value: 2,
          prompts: [
            'Which key IT components are currently reporting metrics beyond their defined thresholds?',
            'List all "critical" or "major" severity alerts for components operating outside normal thresholds in the last 24 hours.',
            'For components that went beyond thresholds yesterday, how long did they remain in that state?',
            'Are there any systems currently oscillating in and out of threshold limits?',
            'What is the trend of threshold breaches over the past 24-48 hours?'
          ]
        }
      ]
    };
